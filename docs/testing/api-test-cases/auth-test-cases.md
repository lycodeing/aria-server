...
| AUTH-015 | 未登录状态调用登出不报错 | 无 token 或已失效 token | POST /logout | HTTP 200（静默幂等，不抛异常） | P1 | |
| AUTH-016 | Token 刷新成功 | 已登录 token | POST /refresh | HTTP 200，返回新 tokenValue，且旧 token 失效 | P0 | |
| AUTH-017 | 未登录刷新被拒 | 无 token | POST /refresh | HTTP 401 UNAUTHORIZED(40100) | P0 | |
...
| AUTH-052 | `/me/login-records` 未实现返回 501 | 已登录 | GET /users/me/login-records | HTTP 501 | P2 | |
| AUTH-053 | `/me/notification-settings` 未实现返回 501 | 已登录 | GET /users/me/notification-settings | HTTP 501 | P2 | |
| AUTH-054 | `/me/mfa` 未实现返回 501 | 已登录 | GET /users/me/mfa | HTTP 501 | P2 | |
| AUTH-055 | `/me/ssh-keys/gpg` 未实现返回 501 | 已登录 | GET /users/me/ssh-keys/gpg | HTTP 501 | P2 | |
| AUTH-056 | `/me/linked-accounts` 未实现返回 501 | 已登录 | GET /users/me/linked-accounts | HTTP 501 | P2 | |
...
| AUTH-082 | 数据域角色不存在返回 404 | - | PUT /roles/999999999/data-scope | HTTP/业务码 NOT_FOUND(40400) | P2 | |
...
| AUTH-088 | 创建菜单 menuType 传任意字符串不校验合法性 | - | POST /menus `{menuName:"x", menuType:"WHATEVER"}` | HTTP 200 成功创建（无枚举硬校验） | P2 | |
...
| AUTH-102 | 非默认配置删除为软删除 | 非默认模型 | DELETE /admin/ai-models/{id}，随后 GET 列表 | HTTP 200；列表中不再出现该记录（deleted_at 生效） | P1 | |
...
| AUTH-105 | 测试连接 RERANKER 类型为 mock（已知行为） | 已配置 RERANKER 模型 | POST /admin/ai-models/{id}/test | HTTP 200，`success:true` 固定返回，不做真实调用（TODO Phase-2） | P2 | **非缺陷，是当前已知实现**，测试断言不应误判为 bug |
...
| AUTH-122 | 删除配置为软删除 | 已创建配置 | DELETE /admin/system-config/{id}，随后 GET 列表 | HTTP 200；列表中不再出现 | P1 | |
