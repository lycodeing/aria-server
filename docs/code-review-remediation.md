...
### 批次 6 — 2026-08-07 — 第 7 轮最终确认 + 已接受残留风险登记

**第 7 轮评审结论**：CONV-8 / AUTH-5 两处 fast-follow 修复正确、无回归（IP 限流计数原子、边界安全、手机号维度限流完整保留、导入清理干净、受影响测试全同步、编译通过）。**整个安全整改循环最终收敛，无 🔴 阻断项。**

**已接受的低危残留风险（登记为技术债，不阻断上线）**：
- **TD-1（IP 限流可被 XFF 伪造绕过）**：CONV-8 的 `HttpRequestUtils.getClientIp` 优先取客户端可伪造的 `X-Forwarded-For`。IP 限流定位为「尽力而为的滥用缓解」而非访问控制，手机号维度限流（60s/号 + 10 分钟锁定）仍是硬防线。加固方向：复用 AUTH-4 的可信代理开关，仅在可信反代后取 XFF、否则用 `remoteAddr`。
- **TD-2（AUTH-5 次级侧信道）**：统一 DISABLED/LOCKED 文案后，仍存在「账号不可用」vs「用户名或密码错误」的响应差异，及「账号不存在不跑 BCrypt」的时序差异。属低危，且「给被禁用的合法用户可操作提示」与「完全不可区分」存在业务权衡。加固方向：统一失败文案 + 对不存在账号补偿性哈希以抹平时序。
