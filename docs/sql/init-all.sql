...
-- 数据库：PostgreSQL 16+（需安装 pgvector 扩展）
--
-- 包含：
--   1. 数据库创建
--   2. pgvector 扩展启用
--   3. cs_auth schema（用户/角色/权限/菜单/部门/数据权限）
--   4. knowledge schema（知识库/文档/Chunk 向量表）
--   5. 菜单和权限种子数据
--   6. 角色和测试用户种子数据
--   7. 默认知识库种子数据
--
...
-- 或：
--   docker exec -i <postgres_container> psql -U aidev -f /tmp/init-all.sql
--
-- 注意：
--   - 执行前确保 pgvector 扩展已安装（postgres:16 官方镜像需额外安装）
--   - 测试用户密码统一为 Test@123456（BCrypt cost=10）
--   - 生产环境务必修改密码哈希
-- ============================================================
...
-- 创建数据库（已存在则跳过）
SELECT 'CREATE DATABASE ai_customerservice OWNER aidev'
WHERE NOT EXISTS (
    SELECT FROM pg_database WHERE datname = 'ai_customerservice'
)\gexec

SELECT 'CREATE DATABASE ai_knowledge OWNER aidev'
WHERE NOT EXISTS (
    SELECT FROM pg_database WHERE datname = 'ai_knowledge'
)\gexec

-- ============================================================
-- 第一部分：认证服务（ai_customerservice 库）
-- ============================================================
\c ai_customerservice

-- pgvector 扩展（认证库无需向量，knowledge 库才需要）
-- 触发器函数（全局）
...
-- ============================================================
-- 12. 文档表
-- 状态机：DRAFT → REVIEW → PUBLISHED → DEPRECATED / FAILED
-- ============================================================
...
COMMENT ON COLUMN knowledge_doc.content_hash IS 'SHA-256(文件内容)，相同内容跳过重摄取';
COMMENT ON COLUMN knowledge_doc.expires_at   IS '文档过期日期，NULL=永久有效；过期后定时任务自动下线';

CREATE OR REPLACE TRIGGER trg_doc_updated BEFORE UPDATE ON knowledge_doc
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- ============================================================
-- 13. Chunk 向量表（pgvector 核心表）
-- ============================================================
...
COMMENT ON TABLE  knowledge_chunk IS 'Chunk 向量表，核心检索单元，使用 pgvector 存储 1024 维 embedding';
COMMENT ON COLUMN knowledge_chunk.kb_id            IS '冗余字段，避免检索时 JOIN knowledge_doc';
COMMENT ON COLUMN knowledge_chunk.doc_status       IS '冗余字段，随 knowledge_doc.status 同步';
...
COMMENT ON COLUMN knowledge_chunk.parent_chunk_id  IS 'Parent-Child 架构：检索用小 chunk，生成用父 chunk';

-- ============================================================
-- 第三部分：种子数据（切回 ai_customerservice）
-- ============================================================
\c ai_customerservice
...
-- 19. 测试用户
-- 密码统一为：Test@123456
-- BCrypt hash（cost=10）：$2b$10$IbF1B4L.Tog2vsx7coxiTehgwstvHi96dSx.7GIgqTnD0yyGcPcO2
-- ⚠️ 生产环境务必修改密码
-- ============================================================
INSERT INTO cs_auth.sys_user (id, username, display_name, email, password_hash, status, provider, must_change_password) VALUES
(1001, 'superadmin', '超级管理员', 'superadmin@example.com',
    '$2b$10$IbF1B4L.Tog2vsx7coxiTehgwstvHi96dSx.7GIgqTnD0yyGcPcO2',
    'active', 'LOCAL', FALSE),
(1002, 'kfmanager',  '客服管理员', 'kfmanager@example.com',
    '$2b$10$IbF1B4L.Tog2vsx7coxiTehgwstvHi96dSx.7GIgqTnD0yyGcPcO2',
    'active', 'LOCAL', FALSE),
(1003, 'kfstaff',    '普通客服',   'kfstaff@example.com',
    '$2b$10$IbF1B4L.Tog2vsx7coxiTehgwstvHi96dSx.7GIgqTnD0yyGcPcO2',
    'active', 'LOCAL', FALSE)
