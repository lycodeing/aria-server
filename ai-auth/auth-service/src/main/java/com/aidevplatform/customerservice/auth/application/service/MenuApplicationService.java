...
import com.aidevplatform.customerservice.auth.application.command.CreateMenuCommand;
import com.aidevplatform.customerservice.auth.application.command.UpdateMenuCommand;
import com.aidevplatform.customerservice.auth.application.result.MenuAdminVO;
import com.aidevplatform.customerservice.auth.application.result.RouteMetaVO;
import com.aidevplatform.customerservice.auth.application.result.RouteVO;
import com.aidevplatform.customerservice.auth.domain.model.menu.Menu;
import com.aidevplatform.customerservice.auth.domain.repository.IMenuRepository;
...
public class MenuApplicationService {
    ...
    public List<RouteVO> buildUserRoutes(Long userId) {
        ...
        return buildRouteNodes(groupByParent, ROOT_PARENT_ID);
    }
    ...
    private List<RouteVO> buildRouteNodes(Map<Long, List<Menu>> groupByParent, Long parentId) {
        ...
        for (Menu menu : children) {
            RouteMetaVO meta = new RouteMetaVO(
                    ...
                    menu.getSortOrder());
            List<RouteVO> subChildren = buildRouteNodes(groupByParent, menu.getId());
    ...
    private List<MenuAdminVO> buildAdminNodes(Map<Long, List<Menu>> groupByParent, Long parentId) {
        List<Menu> children = groupByParent.getOrDefault(parentId, Collections.emptyList());
        List<MenuAdminVO> result = new ArrayList<>(children.size());
        for (Menu menu : children) {
            List<MenuAdminVO> subChildren = buildAdminNodes(groupByParent, menu.getId());
