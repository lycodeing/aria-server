...
public class DocIngestConsumer {
     ...
     * concurrency=2 表示同实例 2 个消费线程并发处理（与原 Redis Streams batch-size 对齐）。
     *
     * @param event 摄取事件 DTO（由 Jackson2JsonMessageConverter 自动反序列化）
     */
    @RabbitListener(queues = "${knowledge.ingest.queue}", concurrency = "2")
    public void consume(DocIngestEvent event) {
