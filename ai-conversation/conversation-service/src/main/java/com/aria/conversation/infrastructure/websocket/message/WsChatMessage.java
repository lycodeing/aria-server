 ...
 * <p>字段说明：
 * <ul>
 *   <li>{@code role}：消息来源角色，通过 {@link MessageRole} 枚举取值，避免魔法字符串</li>
 ...
 * @param role      消息来源角色（{@link MessageRole#USER} 或 {@link MessageRole#AGENT} 的字符串值）
...
public record WsChatMessage(
     ...
     * @return 角色为 {@link MessageRole#USER} 的聊天消息
     */
    public static WsChatMessage fromVisitor(String sessionId, String content, long seq, long timestamp) {
        return new WsChatMessage(WsMessageType.MESSAGE, sessionId,
                MessageRole.USER.getValue(), content, seq, timestamp);
    }
